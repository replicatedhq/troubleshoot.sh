...
Analyzers are YAML specifications that define a set of criteria and operations to run against data collected in a preflight check or support bundle. Each analyzer included will result in either 0 or 1 [outcomes](https://troubleshoot.io). If an analyzer produces zero outcomes, it will not be displayed in the results.
...
*Note: Previously, Analyzers could be specified in a standalone file, with `Kind: Analyzer`, however this syntax is depcrecated in Kots and not supported at all in the CLI tools.*
