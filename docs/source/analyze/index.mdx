...
Optionally, an analyzer can be set to strict. Setting an analyzer  `strict: true` is useful as it provides an additional datapoint for analyzers where `outcomes` should not be ignored.

When `exclude: true` is specified, `exclude` will override `strict` and the analyzer will not execute.
