...
class TroubleshootSupportBundle extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  renderAceEditor = (supportBundleYAML) => {
    import("brace").then((ace) => {
      import("brace/theme/chrome").then(() => {
        import("brace/mode/yaml").then(() => {
          ...
          this.setState({ currentSpecCommand: editor.setValue(supportBundleYAML) });
  ...
  componentDidMount() {
    import("../../static/specs-gen.json").then(module => {
      ...
      this.sendToServer(currentSpec.supportSpecYaml);
      this.renderAceEditor(currentSpec.supportSpecYaml);
    });
  }

  copySpecYamlToClipboard = () => {
    const textField = document.createElement("textarea");
    textField.innerText = this.state.supportBundleYAML;
  ...
  onTryItOut = () => {
    ...
    this.setState({ showCodeSnippet: true, currentCommand: bundleCommand });
  }
      ...
      .then(async (res) => {
        if (method === "POST") {
          ...
          if (!result) {
            ...
            this.setState({ supportBundlePreviewId: result.id }, () => {
              this.onTryItOut();
            });
          }
        }
  ...
  onSpecChange = (value) => {
    this.setState({ supportBundleYAML: value });
  ...
  downloadYaml = () => {
    const filename = "support-bundle.yaml";

    this.download(filename, this.state.supportBundleYAML);
  }
            ...
            <div className="troubleshootSection">
              ...
              {!isMobile &&
                <div className="EditorHeader-wrapper flex flex1 alignItems--center u-marginTop--30">
                  <span className="icon support-small u-marginRight--small" /> <span className="u-fontSize--normal u-fontWeight--bold u-color--biscay"> Support Bundle </span>
                ...
                {!isMobile ?
                  <div className="AbsoluteTest-wrapper flex-column u-width--fourth u-marginRight--10">
                    <div className="Test-wrapper flex-column flex1 u-overflow--hidden">
                      ...
                      <span className="u-color--dustyGray u-fontSize--normal body-copy u-lineHeight--more u-marginTop--15"> First you will need to <a href="/docs" target="_blank" rel="noopener noreferrer" className="u-color--royalBlue u-fontWeight--medium  u-textDecoration--underlineOnHover"> install the support bundle plugin. </a> Once you have the plugin, run the following command to get results against your cluster. </span>
                      {fetchingError ?
                        <div className="ErrorBlock u-marginTop--10"> There was a problem fetching the command. Please <span className="u-color--royalBlue u-fontWeight--medium  u-textDecoration--underlineOnHover" onClick={() => this.sendToServer(this.state.supportBundleYAML)}> try again  </span> </div>
                        :
                        showCodeSnippet &&
                        <div className="flex flex-column u-marginTop--normal u-marginTop--10">
                          <CodeSnippet
                            ...
                            learnMore={<a href="/docs/support-bundle/introduction/" target="_blank" rel="noopener noreferrer" className="u-color--royalBlue u-lineHeight--normal u-fontSize--small u-textDecoration--underlineOnHover">
                              Learn more</a>}
                          >
              ...
              {isMobile && <div className="u-marginTop--10 flex alignItems--center">
                {!copySuccess ?
...
export default TroubleshootSupportBundle;